...
### Changes & Improvements:
 ...
 - The device screen can be viewed live from the Screen Capture window: select the "Live" row at the top of the screenshot list. Clicks, drags and typing are sent to the device as touch and key events, and Ctrl+A, Ctrl+C and Ctrl+V (Cmd on macOS) select all, copy and paste on the device using its own clipboard, and Back / Home / Overview buttons sit beside the image with the stream details. The stream's size, JPEG quality and frame rate cap can be changed in Preferences > Analysis > Android Logcat Settings. Streaming stops when another row is selected or the window is closed, and right clicking the "Live" row offers Reconnect for a stream that stopped on its own.
